...
try
{
	...
	mkfifo(srd_cwr_req_np_str.c_str(),0600);	//fifo is first in first out (a queue)
	if(chmod (srd_cwr_req_np_str.c_str(), 0600) < OK)
	...
	string randomword_str(clientno_ary);

	char swr_crd_np_ary[READ_MAX_LEN] = {0};

	if(read(swr_crd_np_fd, swr_crd_np_ary, READ_MAX_LEN) < OK)
		throw domain_error(LineInfo("read FAILURE", __FILE__,__LINE__));
	string srd_cwr_np_str(swr_crd_np_ary);
	...
	if(srd_cwr_np_fd < OK)
		throw domain_error(LineInfo("open FAILURE", __FILE__, __LINE__));
	int noOfTries = 0;
	cout << endl << endl<< "Game Start" << endl << endl << "You have " << NO_OF_LETTER_GUESS_MAX << " letter guesses to win" << endl << endl;
	
	do
	{
		char guessWordAry[READ_MAX_LEN] = {0};
		if(read(srd_cwr_np_fd, guessWordAry, READ_MAX_LEN) < OK)
			throw domain_error(LineInfo("read FAILURE", __FILE__,__LINE__));
